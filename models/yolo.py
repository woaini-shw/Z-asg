...
class Model(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=True):
        y, dt = [], []
        for m in self.model:
            if m.f != -1:
            ...
            visualize = Path('./feature_visualization')
...
def parse_model(d, ch):
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d,SpatialGate,CARAFE,SELayer]:
            c1, c2 = ch[f], args[0]
            if c2 != no:
            ...
            if m in [BottleneckCSP, C3, C3TR,  C3Ghost]:
                args.insert(2, n)
                n = 1
        elif m is [nn.BatchNorm2d,PixelShuffle]:
            args = [ch[f]]
        elif m is Concat:
