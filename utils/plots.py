...
def check_font(font='Arial.ttf', size=6):
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('feature_visualization')):
    ...
    img= cv2.imread('../datasets/VisDrone2019/val/images/0000001_02999_d_0000005.jpg')
    heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))  
    heatmap = np.uint8(255 * heatmap)
    heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
    heat_img = cv2.addWeighted(img, 1, heatmap, 0.5, 0)
    # heat_img = heatmap * 0.5 + img 　　　　　　        　　　

    plt.figure()
    plt.imshow(heat_img)
    plt.axis('off')
    f = 'feature_map_sum.png'
...
@Timeout(2000)  # known issue https://github.com/ultralytics/yolov5/issues/5611
