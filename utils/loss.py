    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                nwd_ratio = 0.7  # 平衡稀疏  nwd和 iou各取0.5   如果数据集全是小目标的换可以设置1或者0.9，,08 意思为只用nwd
                lbox += (1 - nwd_ratio) * (1.0 - nwd).mean() + nwd_ratio * (1.0 - iou).mean()
                # Objectness
                score_iou = (iou.detach() * nwd_ratio + nwd.detach() * (1 - nwd_ratio)).clamp(0, 1).type(tobj.dtype)  # 这里clamp（0，1）中设置了最小参数必须大于等于0进行
                # ===============================================
